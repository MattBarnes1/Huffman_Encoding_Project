...
void PriorityQueue<T>::insert_with_priority(T theData, int Priority)
{
	...
	{
		if (Ptr == root && Ptr->Priority > Priority)
		{
		...
		else if (Ptr->nxt != 0)
		{
			...
			else if (Ptr->Priority <= Priority && Ptr->nxt->Priority == Priority)
			{
				if (Ptr->nxt->nxt == 0)
				{
					PriorityQueue_LL<T> *Temp = Ptr->nxt;
...
void PriorityQueue<T>::insert_without_priority(PriorityQueue_LL<T>* newNode)
{
	...
	for (Ptr = root; Ptr != 0; Ptr = Ptr->nxt)
	{
		if (Ptr == root && Ptr->Priority > newNode->Priority)
		{
			PriorityQueue_LL<T> *Temp = root;
			root = newNode;
			root->nxt = Temp;
		...
		else if (Ptr->nxt != 0)
		{
			...
			else if (Ptr->Priority <= newNode->Priority && Ptr->nxt->Priority == newNode->Priority)
			{
				if (Ptr->nxt->nxt == 0)
				{
					PriorityQueue_LL<T> *Temp = Ptr->nxt;
